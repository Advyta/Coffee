      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            tl.fromTo(
              headingRef.current, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 1.5, ease: 'power2.in' }
            ).fromTo(
              cardRef.current, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 1.5, ease: 'power1.in' }, '-=0.8'
            );
            observer.disconnect(); // Stop observing once the animations are triggered
          }
        });
      },
    ...
    >
      ...
      <div className='flex flex-col w-full h-full overflow-y-auto'>
        <div className='flex text-center items-center mt-14 md:mt-48 lg:mt-16 justify-center'>
          <h2 ref={headingRef} className='heading-2 mx-4 md:mx-16 px-5 py-2 rounded-xl capitalize backdrop-blur-lg bg-background-40'>Discover our most Popular Coffee Recipes</h2>
        </div>

        <div ref={cardRef} className='grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-8 mt-5 p-4'>
          {recipes.map((recipe) => (
            <div key={recipe._id} className='justify-self-center'>
              <div className='flex flex-col gap-4 lg:gap-8 sm:flex-row bg-background-40 backdrop-blur-sm px-8 py-6 lg:px-14 md:py-9 rounded-xl max-w-xl justify-center'>
                <div className=' flex justify-center sm:basis-1/2'>
                  <img src={recipe.image} alt={recipe.name} className='rounded-full w-24 sm:w-28 md:w-40 lg:w-60 xl:w-80 aspect-[144/83]' />
                </div>
